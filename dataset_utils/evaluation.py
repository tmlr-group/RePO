...
def smiles_to_fingerprint(smiles, n_bits=2048):
    ...
    Note: This uses GetMorganFingerprintAsBitVect which may show a deprecation warning.
    When upgrading RDKit, consider switching to the fingerprint generator approach with
    ...
    # Using the traditional method - may show deprecation warnings
    # but is compatible with older RDKit versions
...
def calculate_novelty(new_smiles_list):
    ...
    # Filter out None values from fingerprints
    known_fps = [fp for fp in [smiles_to_fingerprint(smiles) for smiles in known_smiles_list] if fp is not None]
    new_fps = [fp for fp in [smiles_to_fingerprint(smiles) for smiles in new_smiles_list] if fp is not None]
    
    # Convert to tensor
    known_fps_tensor = fingerprints_to_tensor(known_fps)
    new_fps_tensor = fingerprints_to_tensor(new_fps)
    
    # If no valid fingerprints, return empty array
    if len(known_fps) == 0 or len(new_fps) == 0:
        return np.array([])
    
    dot_product = torch.mm(new_fps_tensor, known_fps_tensor.t())
    norm_new = new_fps_tensor.sum(dim=1).unsqueeze(1)
    norm_known = known_fps_tensor.sum(dim=1).unsqueeze(0)
