...
class XGRPOTrainer(GRPOTrainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                if "solution" in reward_kwargs:
                    ...
                    # MuMo reward functions expect `solution=...` and `original_mol=...` (not target_mol)
                    if self.variant == "mumo":
                        modified_kwargs = dict(reward_kwargs)
                        if "molecule" in modified_kwargs:
                            modified_kwargs["original_mol"] = modified_kwargs.pop("molecule")
                        ...
                        if "molecule" in reward_kwargs:
                            ...
                            output_reward_func = reward_func(
                                prompts=prompts,
                                completions=completions,
                                target_mol=solutions_for_reward,
        ...
        if self.variant == "pure":
            # Pure variant uses the normal attention mask (no span detection)
            solution_inputs.pop("offset_mapping", None)
        else:
