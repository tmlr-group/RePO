...
from rewards import (accuracy_reward, format_reward, get_cosine_scaled_reward,
...
class GRPOScriptArguments(ScriptArguments):
    ...
    # Noisy demonstration knobs (used by the *_noisy_main variants; harmless for default)
    noise_prob: float = field(
        default=0.0,
        metadata={"help": "Probability of replacing a molecule with another from the same batch/bucket"},
    )
    noise_bucket_key: str | None = field(
        default=None,
        metadata={"help": "Column name used to constrain replacements within the same bucket (e.g., 'SubTask')"},
    )
    noise_rebuild: bool = field(
        default=False,
        metadata={"help": "Force rebuilding the noisy dataset cache even if it exists"},
    )
...
def _apply_batch_shuffle_noise(df: pd.DataFrame, seed: int, p: float, bucket_key: str | None) -> pd.DataFrame:
    ...
    apply_mask = (torch.rand(n, generator=g) < p).tolist()

    if bucket_key is not None and bucket_key in df.columns:
        replacement: dict[int, int] = {}
        for _, idx_series in df.groupby(bucket_key).indices.items():
            idxs = list(idx_series)
            if len(idxs) == 1:
                replacement[idxs[0]] = idxs[0]
            else:
                rotated = idxs[1:] + idxs[:1]
                for i, j in zip(idxs, rotated):
                    replacement[i] = j
        orig = df["molecule"].tolist()
        new_vals = orig.copy()
        for i in range(n):
            if apply_mask[i]:
                j = replacement.get(i, i)
                new_vals[i] = orig[j]
        df["molecule"] = new_vals
    else:
        idxs = list(range(n))
        rotated = idxs[1:] + idxs[:1]
        orig = df["molecule"].tolist()
...
def main(script_args, training_args, model_args, variant: str = "default"):
    ...
    if variant == "math":
        ...
        if script_args.subtask_selection == ["AddComponent", "SubComponent", "DelComponent"]:
            ...
            data = json.load(open(f"data/TRAIN_multi_prop/IND_sft_train_data_{script_args.subtask_selection}.json", "r"))
            df = pd.DataFrame(data)
            ds = Dataset.from_pandas(df)
            dataset = DatasetDict({"train": ds})
            dataset = dataset.rename_column("instruction", "problem")
            ...
            # fallback to user-provided dataset_name / config
            dataset = load_dataset(script_args.dataset_name, name=script_args.dataset_config)
        train_dataset = None
        eval_dataset = None
    else:
        # OpenMolIns Property Optimization (default + noisy_demo)
        if script_args.subtask_selection == ["LogP", "MR", "QED"]:
            ...
            if variant == "noisy_demo":
                noise_prob = float(getattr(script_args, "noise_prob", 0.0))
                noise_bucket_key = getattr(script_args, "noise_bucket_key", None)
                noise_rebuild = bool(getattr(script_args, "noise_rebuild", False))

                cache_path = None
                if noise_prob > 0.0:
                    kind = "bucket" if isinstance(noise_bucket_key, str) and len(noise_bucket_key) > 0 else "global"
                    p_tag = f"p{int(round(noise_prob * 100))}"
                    cache_name = f"train.noisy_{kind}_{p_tag}.csv"
                    cache_path = os.path.join(dataset_dir, cache_name)

                if cache_path is not None and os.path.exists(cache_path) and not noise_rebuild:
                    logger.info(f"[noisy_demo] Loading cached noisy dataset: {cache_path}")
                    df = pd.read_csv(cache_path)
                else:
                    if noise_prob > 0.0:
                        df_noisy = _apply_batch_shuffle_noise(
                            df=df.copy(),
                            seed=int(training_args.seed),
                            p=noise_prob,
                            bucket_key=noise_bucket_key if isinstance(noise_bucket_key, str) else None,
                        )
                        if cache_path is not None:
                            logger.info(f"[noisy_demo] Saving cached noisy dataset to: {cache_path}")
                            os.makedirs(os.path.dirname(cache_path), exist_ok=True)
                            df_noisy.to_csv(cache_path, index=False)
                        df = df_noisy
    ...
    if dataset is not None and script_args.dataset_name == "FreedomIntelligence/medical-o1-verifiable-problem":
        dataset = dataset.rename_columns({"Open-ended Verifiable Question": "problem", "Ground-True Answer": "solution"})
    ...
    if dataset is not None:
        ...
        if isinstance(train_split, str) and train_split in dataset:
            split = dataset[train_split]
            column_names = set(getattr(split, "column_names", []) or [])
    ...
    if variant == "math":
        ...
        REWARD_FUNCS_REGISTRY = {
            ...
            "format": lambda prompts, completions, **kwargs: format_reward(completions=completions),
            "reasoning_steps": lambda prompts, completions, **kwargs: reasoning_steps_reward(completions=completions),
            "cosine": lambda prompts, completions, solutions=None, **kwargs: get_cosine_scaled_reward(
                min_value_wrong=script_args.cosine_min_value_wrong,
                max_value_wrong=script_args.cosine_max_value_wrong,
                min_value_correct=script_args.cosine_min_value_correct,
                max_value_correct=script_args.cosine_max_value_correct,
            ...
            "repetition_penalty": lambda prompts, completions, **kwargs: get_repetition_penalty_reward(
                ngram_size=script_args.repetition_n_grams, max_penalty=script_args.repetition_max_penalty
            )(completions=completions),
...
def run(variant: str = "default"):
    ...
    if variant == "random_mask":
        from configs_random_mask import RandomMaskGRPOConfig as config_cls

    parser = TrlParser((GRPOScriptArguments, config_cls, ModelConfig))
